#!/usr/bin/env python3
"""Render the benchmark JSON as a README-friendly static and animated SVG."""
...
def make_svg(document: dict, animated: bool) -> str:
    ...
    parts = [
        ...
        '@keyframes grow{from{transform:scaleX(0)}to{transform:scaleX(1)}} .animated{transform-box:fill-box;transform-origin:left center;animation:grow .9s cubic-bezier(.2,.8,.2,1) both} @media(prefers-reduced-motion:reduce){.animated{animation:none!important}}</style>',
    ...
    for index, row in enumerate(rows):
        ...
        for lane, engine in enumerate(("libzip-c", "kzip-rust")):
            ...
            parts.append(
                ...
                f'data-target-width="{bar_width:.2f}" style="animation-delay:{delay:.3f}s"/>'
            )
...
def main() -> int:
    if len(sys.argv) != 4:
        print("usage: render.py <benchmark-json> <static-svg> <animated-svg>", file=sys.stderr)
        return 2
